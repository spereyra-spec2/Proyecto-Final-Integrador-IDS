...
def reporte_alumnos_pdf(idCurso):
    ...
    return send_file(
        ...
        as_attachment=True, 
        download_name='reporte_alumnos_curso_{idCurso}.pdf'
    )

#--------------------------------------------------------------------------------------------------------
# GET /api/cursos/{idCurso}/reporte-promedio
# Genera y exporta un documento PDF con el porcentaje de aprobados de un curso
#--------------------------------------------------------------------------------------------------------
@cursos_bp.route('/<int:idCurso>/reporte-promedio', methods=['GET'])
def reporte_promedio_general_pdf(idCurso):
    ...
    try:
        ...
        funciones.registrar_auditoria(cursor, padron_operador, f"Exportó PDF del promedio de aprobación del curso {idCurso}")
        conn.commit()
    except Exception as e:
        return errors.server_error(str(e))
    finally:
    ...
    story.append(Paragraph("<b>ACADEMIQ - REPORTE DE PROMEDIO GENERAL</b>", styles['Heading1']))
    story.append(Paragraph(f"Curso ID: {idCurso} | Vista de Calificaciones", styles['Normal']))
    ...
    return send_file(
        ...
        mimetype='application/pdf', 
        as_attachment=True, 
        download_name=f'promedio_general_curso_{idCurso}.pdf'
    )
...
def create_curso():
    ...
    try:
        ...
        error_unicidad = validaciones.verificar_unicidad_curso(cursor, nombre, codigo, cuatrimestre)
        if error_unicidad:
...
def patch_curso(idCurso):
    ...
    try:
        ...
        # 1. Agregamos cuatrimestre a la consulta inicial
        cursor.execute("SELECT nombre, codigo, cuatrimestre FROM Curso WHERE idCurso = %s", (idCurso,))
        ...
        # 2. Obtenemos el cuatrimestre que quedará (el nuevo enviado o el viejo de la BD)
        nuevo_cuatrimestre = data.get("cuatrimestre", curso_actual["cuatrimestre"]).strip()
        
        error_formato = validaciones.validar_curso_datos(nuevo_nombre, nuevo_codigo)
        if error_formato:
            return error_formato
  
        # 3. Agregamos nuevo_cuatrimestre e idCurso a la validación de unicidad
        error_unicidad = validaciones.verificar_unicidad_curso(cursor, nuevo_nombre, nuevo_codigo, nuevo_cuatrimestre, idCurso)
        if error_unicidad:
            return jsonify(error_unicidad), 409
